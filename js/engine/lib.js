...
export const LEMMAS = [
  ...
  // ── Listes ──────────────────────────────────────────────────────────────
  { name: 'nil_append', stmt: '∀ (l : List ℕ), [] ++ l = l', kind: 'axiom', simp: true, group: 'Listes',
    doc: 'Concaténer à la liste vide ne fait rien. Première équation de définition de `++`.' },
  { name: 'cons_append', stmt: '∀ (x : ℕ) (l m : List ℕ), (x :: l) ++ m = x :: (l ++ m)', kind: 'axiom', simp: true, group: 'Listes',
    doc: 'La tête sort de la concaténation. Seconde équation de définition de `++` : elle récurse sur la liste de gauche, contrairement à `+` qui récurse à droite.' },
  { name: 'append_nil', stmt: '∀ (l : List ℕ), l ++ [] = l', kind: 'thm', simp: true, group: 'Listes',
    doc: 'La symétrique de `nil_append`, et comme pour `zero_add` elle n’est pas gratuite : il faut une récurrence.' },
  { name: 'append_assoc', stmt: '∀ (l m n : List ℕ), (l ++ m) ++ n = l ++ (m ++ n)', kind: 'thm', group: 'Listes',
    doc: 'Concaténer est associatif. En revanche ce n’est pas commutatif — l’ordre des éléments est précisément ce qu’une liste retient.' },
  { name: 'length_nil', stmt: 'length [] = 0', kind: 'axiom', simp: true, group: 'Listes',
    doc: 'La liste vide n’a aucun élément.' },
  { name: 'length_cons', stmt: '∀ (x : ℕ) (l : List ℕ), length (x :: l) = succ (length l)', kind: 'axiom', simp: true, group: 'Listes',
    doc: 'Un élément de plus, un successeur de plus. C’est le pont entre les listes et ℕ.' },
  { name: 'length_append', stmt: '∀ (l m : List ℕ), length (l ++ m) = length l + length m', kind: 'thm', group: 'Listes',
    doc: 'La longueur transforme la concaténation en addition. En algèbre, on appelle ça un morphisme.' },
  { name: 'reverse_nil', stmt: 'reverse [] = []', kind: 'axiom', simp: true, group: 'Listes',
    doc: 'Retourner le vide donne le vide.' },
  { name: 'reverse_cons', stmt: '∀ (x : ℕ) (l : List ℕ), reverse (x :: l) = reverse l ++ [x]', kind: 'axiom', simp: true, group: 'Listes',
    doc: 'La tête part à la fin. Définition de `reverse` — et la raison pour laquelle cette version est quadratique.' },
  { name: 'reverse_append', stmt: '∀ (l m : List ℕ), reverse (l ++ m) = reverse m ++ reverse l', kind: 'thm', group: 'Listes',
    doc: 'Retourner une concaténation échange les morceaux. Indispensable au boss du monde 10.' },
  { name: 'reverse_reverse', stmt: '∀ (l : List ℕ), reverse (reverse l) = l', kind: 'thm', group: 'Listes',
    doc: 'Retourner deux fois ramène au départ. Le théorème le plus satisfaisant du jeu.' },
